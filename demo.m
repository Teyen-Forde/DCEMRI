...
%==================================================================
impulsive = 0; % 1 for Gaussian and 0 for impulsive Sparse matrices
idx=0;
clear Aerr
for spr = 0.001:0.005:0.4; % sparsity ratio: #nonzeros/m/n
for pr =0.001:0.005:0.4
rB = round(m*pr); % rank of Low-Rank matrix
%==================================================================
...
% tau=1/sqrt(sqrt(m*n));
% t =tau / (1 + tau);
[Bhat, Ahat, iter] = inexact_alm_rpca(C);
% disp('errLR');isame(B,Bhat)
% disp('errSP');isame(A,Ahat)
% disp('Relerr');isame(C,Ahat+Bhat)
idx=idx+1;
Aerr(idx)=isame(B,Bhat);
end
end
%% algorithm
% opts = [];
