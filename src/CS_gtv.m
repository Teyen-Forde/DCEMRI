...
mus      = opts.mus;%Augmented Lagrangian penalty parameter
mut      = opts.mut;%Augmented Lagrangian penalty parameter
%normalize data
dscale  = norm(abs(b(:)));
b       = b / dscale;
...
gamma = 1.618;
%%
AA = @(x) adjDx(Dx(x))+adjDy(Dy(x))+adjDt(Dt(x))+FT(F(x));
maxcgiter = 16;
tol = 5e-4;
% z=0; zx=0; zy=0;
v=0; vx=0; vy=0; vt=0;
...
while ~converged
    ...
    %%
    zx  = softL2x(Dx(M)-vx,mus);
    zy  = softL2y(Dy(M)-vy,mus);
    zt  = softL2t(Dt(M)-vt,mut);
    z   = proxL2(F(M)-v);
    %%
    %% update M using cgsolve
    bb=FT(z+v)+adjDx(zx+vx)+adjDy(zy+vy)+adjDt(zt+vt);
...
function y=softL2t(x,p)
if p
sz = size(x,3);
nx=sqrt(sum(x.^2,3));
sf=max(nx-p,0)./nx;
y = x.*repmat(sf,[1,1,sz]);
else
    y=x;
end
% y(isnan(y))=0;
